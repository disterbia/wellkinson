CREATE TABLE users (
    ...
    user_type TINYINT NOT NULL COMMENT '사용자 타입 0:해당없음 1:파킨슨 환자 2:보호자',
...
CREATE TABLE alarms (
    ...
    type TINYINT NOT NULL COMMENT '타입코드 1: 운동 2:약 3:수면',
...
CREATE TABLE notifications (
    id INT AUTO_INCREMENT PRIMARY KEY,
    uid INT NOT NULL  COMMENT '유저아이디',
    type TINYINT  NOT NULL  COMMENT '알람타입',
    body TEXT  NOT NULL  COMMENT '알람내용',
...
CREATE TABLE main_services(
    ...
    level TINYINT NOT NULL DEFAULT 0 COMMENT ' 0: 활성화 ,10:비활성화',
    created DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
)engine=InnoDB default charset utf8 COMMENT = '이용하고 싶은 서비스 베이스';

CREATE TABLE use_services(
    id INT AUTO_INCREMENT PRIMARY KEY,
    uid INT NOT NULL  COMMENT '유저아이디', 
    service_id INT NOT NULL COMMENT '서비스 아이디',
    title VARCHAR(40) NOT NULL COMMENT '서비스명',
    FOREIGN KEY (uid) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (service_id) REFERENCES main_services(id) ON DELETE CASCADE,
    created DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
)engine=InnoDB default charset utf8 COMMENT = '유저별 이용하고 싶은 서비스';